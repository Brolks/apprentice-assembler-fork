...
local function apply_effects_if_changed(struct, effects)
  if not struct then
    return false
  end
  ...
  local old_effects = struct.applied_effects or ZERO_EFFECTS
  if old_effects.speed == next_effects.speed
    and old_effects.productivity == next_effects.productivity
    and old_effects.consumption == next_effects.consumption
    and old_effects.pollution == next_effects.pollution
    and old_effects.quality == next_effects.quality then
    return true
  end

  struct.applied_effects = next_effects
  return safe_set_effects(struct, next_effects)
end
...
script.on_configuration_changed(function(data)
  ...
  for _, struct in pairs(storage.structs) do
    ...
    struct.applied_effects = struct.applied_effects or ZERO_EFFECTS
  end
end)
...
local function finished_crafting(struct)
  ...
  if struct.products_finished < MAX_BOOST then
    local next_boost = math.min(MAX_BOOST, struct.products_finished + BOOST_STEP)
    if next_boost ~= struct.products_finished then
      struct.products_finished = next_boost
      apply_effects_if_changed(struct, {
...
local function stopped_working(struct)
  ...
  apply_effects_if_changed(struct, ZERO_EFFECTS)
  reset_offering_1(struct)
end
